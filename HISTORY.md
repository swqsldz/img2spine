...
## 2026-09-11 — 颈根修正与 README 动图

- 用 imagegen 补全侦察员颈根与喉部，重新配准头部和颈部支点；前领覆盖扩展至内衫两侧，消除领口内露出的橙色填充与后颈外溢。
- 更新可复建素材、来源记录和母图对比；四动作重新进行官方 runtime 数值检查与 48 帧浏览器复核。
- 新增 `npm run animate`，从最终资源渲染 GIF/WebP，整段统一裁剪并保留时长及哈希；README 展示机器人挥手、野猪奔跑和侦察员行走。
- 将完整喉部覆盖、内衫遮挡及配准后的支点复查补入工作流。
